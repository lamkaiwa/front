...
import { getVideoPageByUser, deleteVideoById, searchCountMine } from '../../../api/video'
  ...
  methods: {
    deleteVideo(id) {
      this.$confirm('确定要删除吗?', '提示').then(async () => {
        let result = await deleteVideoById(id, this.$store.state.user.token)
    ...
    async init(page = 1, keyWord = '', sortBy = '') {
      let result = await getVideoPageByUser({ userId: this.$store.state.user.id, page, words: keyWord, sortBy }, this.$store.state.user.token)
      this.videos = result.data
      let result1 = await searchCountMine(keyWord, this.$store.state.user.id, this.$store.state.user.token)
      if (result1.code == 200) {
        this.count = result1.data
      }
    }
  },
  created() {
